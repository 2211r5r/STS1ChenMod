...
public class MultiHitPower_temp extends BasePower {
    // Power唯一ID
    public static final String POWER_ID = ChenMod.makeID(MultiHitPower_temp.class.getSimpleName());
    // 本地化文本
    ...
    // 构造方法：宿主 + 打出次数（比如2=打2次，3=打3次）
    public MultiHitPower_temp(AbstractCreature owner, final int amount) {
    ...
    public void onUseCard(final AbstractCard card, final UseCardAction action) {
        ...
        if (!card.purgeOnUse && card.type == AbstractCard.CardType.ATTACK) {
            ...
            // 2. 重复打出 hitTimes-1 次（原牌已打1次，总计hitTimes次）
