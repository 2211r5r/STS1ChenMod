...
public class JueYingEffect extends AbstractGameEffect {
    // 绝影刀光专属参数（可全局调整）
    private static final float LENGTH_MULTIPLIER = 75.0f; // 刀光长度系数（越大越长）
    ...
    private static final Color MAIN_COLOR = new Color(1.2f, 0.0f, 0.0f, 1.0f);    // 主色（加深红色）
    private static final Color SECOND_COLOR = new Color(1.0f, 0.2f, 0.2f, 0.8f); // 副色（深红半透明）
    ...
    // 构造方法：极简调用（仅传目标坐标）
    public JueYingEffect(float targetCX, float targetCY) {
        this(targetCX, targetCY, LENGTH_MULTIPLIER, WIDTH_MULTIPLIER);
    }

    // 构造方法：自定义长度系数（备用）
    public JueYingEffect(float targetCX, float targetCY, float lengthMultiplier, float widthMultiplier) {
